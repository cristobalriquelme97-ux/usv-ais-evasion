# Ejecuta un escenario de simulación y devuelve los resultados estructurados.
...
def list_scenarios() -> list[dict[str, Any]]:
    """Retorna los escenarios disponibles para la interfaz."""
    ...
    return sorted(
        ...
        key=lambda item: str(
            item.get("name", item.get("output_file", ""))
        ),
    )
...
def get_scenario_metadata(
    ...
    """Obtiene metadatos y resultados esperados de un escenario."""
...
def resolve_scenario_file(
    ...
    """Resuelve un nombre, stem o archivo txt existente."""
    ...
    raise FileNotFoundError(
        f"No se encontro el escenario: {scenario_name}"
    )
...
def run_scenario(
    ...
    state_machine = NavigationStateMachine(
        config=StateMachineConfig(
            ...
            stand_on_action_delay_s=(
                config.stand_on_action_delay_s
            ),
    ...
    active_evasive_course_deg: float | None = None
    active_evasive_speed_kn: float | None = None
    active_avoidance_decision: dict[str, Any] | None = None

    commanded_course_deg = scenario_cfg.usv_cog_deg
    commanded_speed_kn = scenario_cfg.usv_sog_kn
    replan_count = 0
    ...
    for frame in source.read_frames(
        ...
        critical_assessment = select_most_critical_assessment(
            assessments
        )
            ...
            return_course_evaluation = (
                simulate_course_candidate_against_targets(
                    ...
                    candidate_course_deg=(
                        route_manager.get_return_course()
                    ),
                    safety_radius_m=config.safety_radius_m,
                    time_horizon_s=config.time_horizon_s,
        ...
        speed_only_plan_active = bool(
            active_avoidance_decision
            and str(
                active_avoidance_decision.get("action", "")
        ...
        if cancel_stand_on_speed_plan:
            ...
            speed_only_plan_active = False

        # La evaluación original de rumbo activo se conserva para planes
        # give-way. Un plan stand-on actúa solo sobre velocidad, por lo que
        # no debe marcarse inseguro por simular el rumbo con la SOG anterior.
        if speed_only_plan_active:
        ...
        replanning_info = determine_replanning_need(
            ...
            critical_assessment=critical_assessment,
            active_evasive_course_deg=active_evasive_course_deg,
            active_avoidance_decision=active_avoidance_decision,
            active_course_evaluation=active_course_evaluation,
        )
        ...
        if (
            ...
            avoidance_decision = recommend_avoidance_maneuver(
                ...
                stand_on_speed_factors=(
                    config.stand_on_speed_factors
                ),
                speed_change_rate_kn_s=(
                    config.speed_change_rate_kn_s
                ),
            )

            if avoidance_decision["maneuver_required"]:
                if replanning_info["trigger"] != "initial_plan":
                    replan_count += 1

                avoidance_decision = decorate_avoidance_decision(
                    avoidance_decision,
                    critical_assessment=critical_assessment,
                    current_time_s=frame.timestamp_s,
                ...
                active_evasive_course_deg = float(
                    avoidance_decision["recommended_course_deg"]
                )
                active_evasive_speed_kn = float(
                    avoidance_decision.get(
                        "recommended_speed_kn",
                        ownship["sog_kn"],
                    )
                )
                active_avoidance_decision = dict(
                    avoidance_decision
                )
        ...
        if current_state == "AVOIDING_TARGET":
            commanded_course_deg = (
                ...
                else float(ownship["cog_deg"])
            )
            commanded_speed_kn = (
                active_evasive_speed_kn
                if active_evasive_speed_kn is not None
                else float(ownship["sog_kn"])
            )
            ...
            if active_evasive_speed_kn is not None:
                commanded_speed_kn = active_evasive_speed_kn
            ...
            commanded_course_deg = float(ownship["cog_deg"])
            commanded_speed_kn = float(ownship["sog_kn"])
        ...
        ownship = advance_vessel_state_with_course_and_speed_command(
            vessel=ownship,
            commanded_course_deg=commanded_course_deg,
            commanded_speed_kn=commanded_speed_kn,
            dt_s=scenario_cfg.step_s,
            turn_rate_deg_s=scenario_cfg.usv_turn_rate_deg_s,
            speed_change_rate_kn_s=config.speed_change_rate_kn_s,
        )
    ...
    if save_results:
        results_dir = (
            Path(scenario_file).parent.parent / "results"
        )
...
def _build_step(
        ...
        targets.append(
            {
                ...
                "true_bearing_deg": bearing_info.get(
                    "true_bearing_deg"
                ),
                "relative_bearing_deg": bearing_info.get(
                    "relative_bearing_deg"
                ),
                "side": bearing_info.get("side"),
                "encounter_type": classification.get(
                    "encounter_type"
                ),
                "encounter_name": classification.get(
                    "encounter_name"
                ),
                "ownship_role": classification.get(
                    "ownship_role"
                ),
                "should_maneuver": classification.get(
                    "should_maneuver"
                ),
    ...
    critical_mmsi = None
    if critical_assessment is not None:
        critical_mmsi = critical_assessment["target"].get(
            "mmsi"
        )
    ...
    if active_course_evaluation is not None:
        active_course_status = {
            "candidate_is_safe": active_course_evaluation[
                "candidate_is_safe"
            ],
            "global_min_distance_m": active_course_evaluation[
                "global_min_distance_m"
            ],
            "blocking_target_mmsi": active_course_evaluation[
